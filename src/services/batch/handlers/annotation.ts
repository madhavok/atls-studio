...
function isChunkSuspect(store: ContextStoreApi): (hash: string) => boolean {
  return (hash: string) => {
    ...
    // Also check staged snippets — they may have been promoted from dormant
    if ('stagedSnippets' in store) {
      for (const [, staged] of (store as any).stagedSnippets as Map<string, any>) {
        if (staged.hash?.startsWith(short) || staged.shortHash === short) {
          return false; // staged snippets are never suspect (freshly read)
        }
      }
    }
    return false;
...
export const handleAnnotate: OpHandler = async (params, ctx) => {
  const hash = params.hash as string;
  const note = (params.note ?? params.content) as string;
  if (!hash) return err('annotate: ERROR missing hash param');
  if (!note) return err('annotate: ERROR missing note/content param');
