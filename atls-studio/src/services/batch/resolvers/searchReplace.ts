...
export const resolveSearchReplace: IntentResolver = (
  ...
  const isConcreteGlob = fileGlob && !fileGlob.includes('*') && !fileGlob.includes('?');
  ...
  for (let i = 0; i < maxMatches; i++) {
    ...
    // If file_glob is a concrete path (no wildcards), use it directly.
    // Otherwise, bind file_path from search results via dataflow.
    if (isConcreteGlob) {
