...
- line_edits discipline: for a file's **first edit** in a session, read exact lines first (read.lines or read.context) then build the patch. For **subsequent edits** to the same file after a successful write, the system tracks live content — use h:refs from the edit result and read.lines(ref:"h:NEW:LL-LL") only if you need a different span; do NOT full-read again. Never guess ranges from memory. Edits apply **sequentially in array order** — each line number targets the file state after all prior edits (insert +N shifts subsequent targets by +N). Count braces in braced languages so replacement blocks balance. Prefer anchor for nested / scope-sensitive edits. For simple spans after a fresh read, line+count+action:"replace" — no old text needed. For 200+ line files, always derive line numbers from read.lines output.
...
### Model Discipline
- line edits: for first touch, read.lines (or read.context) on the target range before constructing replacement text; after a successful edit, chain from h:NEW — do not re-read the same file. Brace-check replacements in \`{\`/\`}\` languages; use anchors when nesting/scope makes line math unsafe
...
- read before **first** write: ensure at least one prior read (any turn) before the first edit to a file. After a successful edit, the system tracks live state — do not re-read; chain from h:NEW refs. Re-read only on stale_hash / authority_mismatch errors or suspect refs
