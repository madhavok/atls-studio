...
// ---------------------------------------------------------------------------
// split_match helpers: AST-based match arm extraction
// ---------------------------------------------------------------------------

#[allow(dead_code)]
pub(crate) struct MatchArmInfo {
    pub pattern: String,
    pub body: String,
    pub start_line: u32,
    pub end_line: u32,
...
pub(crate) struct SharedParam {
    pub name: String,
    pub type_hint: String,
}

/// Tree-sitter node kinds that represent function/method declarations per language.
fn function_node_kinds(lang: atls_core::Language) -> &'static [&'static str] {
    use atls_core::Language::*;
    match lang {
        ...
        C | Cpp => &["function_definition", "function_declarator"],
...
/// Find a function/method node by name in the AST (recursive walk).
/// Language-aware: matches on the appropriate tree-sitter node kinds per language.
pub(crate) fn find_function_node<'a>(
    node: &tree_sitter::Node<'a>,
    source: &str,
    function_name: &str,
    lang: atls_core::Language,
) -> Option<tree_sitter::Node<'a>> {
    let kinds = function_node_kinds(lang);
    find_function_node_inner(node, source, function_name, kinds)
}

/// Extract function name from a node, handling language-specific nesting.
/// C/C++ `function_definition` stores the name inside declarator→declarator→identifier
/// rather than a direct `name` field.
fn extract_function_name<'a>(node: &tree_sitter::Node<'a>, source: &str) -> Option<String> {
    // Direct `name` field (Rust, Go, Python, Java, TS/JS, C#)
    if let Some(name_node) = node.child_by_field_name("name") {
        return ts_node_text(&name_node, source);
    }
    ...
    // function_declarator itself: check its declarator child
...
/// Walk down C/C++ declarator nesting to find the identifier name.
fn extract_name_from_declarator(node: &tree_sitter::Node, source: &str) -> Option<String> {
    if node.kind() == "identifier" {
        return ts_node_text(node, source);
    }
    // function_declarator → declarator field → identifier
    if let Some(inner) = node.child_by_field_name("declarator") {
        return extract_name_from_declarator(&inner, source);
    }
    // pointer_declarator → declarator field
    if let Some(inner) = node.child_by_field_name("name") {
        return ts_node_text(&inner, source);
    }
    None
}

fn find_function_node_inner<'a>(
    node: &tree_sitter::Node<'a>,
    source: &str,
    function_name: &str,
    kinds: &[&str],
    ...
    let mut cursor = node.walk();
    for child in node.children(&mut cursor) {
        if let Some(found) = find_function_node_inner(&child, source, function_name, kinds) {
            return Some(found);
        }
    }
    None
...
/// Find all match expressions inside a function node.
/// Returns (start_line_1based, end_line_1based, scrutinee_text, arms).
pub(crate) struct MatchExprInfo {
    pub start_line: u32,
    pub end_line: u32,
...
pub(crate) fn find_match_expressions(
    fn_node: &tree_sitter::Node,
    source: &str,
) -> Vec<MatchExprInfo> {
    let mut results = Vec::new();
    collect_match_expressions(fn_node, source, &mut results);
    results.sort_by_key(|m| m.start_line);
    results
}

pub(crate) fn collect_match_expressions(
    node: &tree_sitter::Node,
    source: &str,
    results: &mut Vec<MatchExprInfo>,
) {
    if node.kind() == "match_expression" {
        ...
        let scrutinee = node.child_by_field_name("value")
            .and_then(|n| ts_node_text(&n, source))
            ...
            if child.kind() == "match_block" {
                ...
                for arm_node in child.children(&mut arm_cursor) {
                    if arm_node.kind() == "match_arm" {
                        if let Some(arm_info) = extract_match_arm(&arm_node, source) {
                            arms.push(arm_info);
                        }
                    }
                }
    ...
    let mut cursor = node.walk();
    for child in node.children(&mut cursor) {
        collect_match_expressions(&child, source, results);
    }
}

/// Extract pattern and body from a single match_arm node.
pub(crate) fn extract_match_arm(node: &tree_sitter::Node, source: &str) -> Option<MatchArmInfo> {
    let start_line = node.start_position().row as u32 + 1;
    let end_line = node.end_position().row as u32 + 1;
    let full_text = ts_node_text(node, source)?;

    // Split on `=>` to get pattern and body
    let arrow_pos = full_text.find("=>")?;
    let pattern = full_text[..arrow_pos].trim().to_string();
    let body_raw = full_text[arrow_pos + 2..].trim();
    // Strip trailing comma if present
    let body = body_raw.trim_end_matches(',').trim().to_string();

    Some(MatchArmInfo { pattern, body, start_line, end_line })
}

/// Detect shared state: function parameters and let bindings before the match.
pub(crate) fn detect_shared_state(
    fn_node: &tree_sitter::Node,
    source: &str,
    ...
    // Extract function parameters from the "parameters" field
    if let Some(params_node) = fn_node.child_by_field_name("parameters") {
        ...
        for child in params_node.children(&mut cursor) {
            if child.kind() == "parameter" || child.kind() == "self_parameter" {
                let param_text = ts_node_text(&child, source).unwrap_or_default();
                let (name, type_hint) = parse_rust_param(&param_text);
                if !name.is_empty() && name != "self" {
                    params.push(SharedParam { name, type_hint });
                }
            }
        }
    }

    // Walk function body for let bindings before the match.
    // Only include bindings with explicit type annotations — untyped locals
    // can't be represented in handler signatures, so they're left inline.
    if let Some(body_node) = fn_node.child_by_field_name("body") {
        let mut cursor = body_node.walk();
        for child in body_node.children(&mut cursor) {
            let child_line = child.start_position().row as u32 + 1;
            if child_line >= match_start_line { break; }
            if child.kind() == "let_declaration" {
                if let Some(pat) = child.child_by_field_name("pattern") {
                    let name = ts_node_text(&pat, source).unwrap_or_default();
                    if let Some(type_node) = child.child_by_field_name("type") {
                        let type_hint = ts_node_text(&type_node, source)
                            .unwrap_or_default();
...
/// Parse a Rust parameter like `app: AppHandle` into (name, type).
pub(crate) fn parse_rust_param(param_text: &str) -> (String, String) {
    if let Some(colon_pos) = param_text.find(':') {
        ...
        (param_text.trim().to_string(), "auto".to_string())
    }
}

/// Detect the return type of a function from its AST node.
pub(crate) fn detect_return_type(fn_node: &tree_sitter::Node, source: &str) -> Option<String> {
    fn_node.child_by_field_name("return_type")
        .and_then(|n| ts_node_text(&n, source))
}

/// Detect whether a function is async.
pub(crate) fn detect_is_async(fn_node: &tree_sitter::Node, source: &str) -> bool {
    let text = ts_node_text(fn_node, source).unwrap_or_default();
    text.starts_with("async ") || text.starts_with("pub async ") || text.contains(" async fn ")
}

/// Convert a match arm pattern to a handler function name.
/// e.g. `"context"` -> `handle_context`, `"code_search"` -> `handle_code_search`
pub(crate) fn arm_to_handler_name(pattern: &str) -> String {
    let clean = pattern.trim_matches('"').trim_matches('\'')
        .replace(|c: char| !c.is_alphanumeric() && c != '_', "_")
        .trim_matches('_').to_string();
    if clean.is_empty() {
        "handle_unknown".to_string()
    } else {
        format!("handle_{}", clean)
    }
}
