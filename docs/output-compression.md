...
| SP3 | **UHPP shapes** (`:sig`, `:fold`, `:dedent`, `:imports`, `:exports`, `:head(N)`, `:tail(N)`, `:grep(pat)`) | [utils/uhppTypes.ts](../atls-studio/src/utils/uhppTypes.ts), [src-tauri/src/hash_resolver.rs](../atls-studio/src-tauri/src/hash_resolver.rs), [src-tauri/src/shape_ops.rs](../atls-studio/src-tauri/src/shape_ops.rs) | Structural view selected with one modifier. `:sig` is file-type aware: code returns declaration skeletons; markdown returns a heading outline with `[start-end]` section ranges (same `NNNN\|…` row format, fence-safe) instead of the code-oriented `head(20)` fallback |
...
| # | Mechanism | Lives in | Effect |
|---|-----------|----------|--------|
| TR1 | **`deflateToolResults`** — inline tool results replaced with `[h:XXXX …]` pointers when a matching engram exists; `addChunk` gated by `isContentArchiveWorthy` for `batch` tool_results | [services/historyCompressor.ts](../atls-studio/src/services/historyCompressor.ts) | Avoids duplicate content between history and WM; skips chunk creation entirely for status-only batch shells |
| TR2 | **`compressToolLoopHistory`** — tool results over `COMPRESSION_THRESHOLD_TOKENS` (100 base; 200 for `system.*`/`verify.*`) deflated to pointers and registered as chunks; new-chunk path also gated by `isContentArchiveWorthy` | [historyCompressor.ts](../atls-studio/src/services/historyCompressor.ts) | Large multipliers: a 2000-token result becomes a ~20-token pointer; no-signal batch shells never enter the manifest |
...
| TR4 | **Failed-step dedupe (Rule A)** — identical `(op, message)` failures in one batch collapse to exemplar + `[FAIL] +N identical (...)` tail | [services/batch/resultFormatter.ts](../atls-studio/src/services/batch/resultFormatter.ts) | Tool_result drops to ~1/N for N-repeated failures; model keeps the full exemplar for steering |
| TR5 | **FileView-merge pointer (Rule B)** — successful `read.lines`/`read.shaped` whose content will be held by a pinned FileView emit `→ merged into h:<RET> [A-B] \| see ## FILE VIEWS` instead of the raw line dump | [services/batch/resultFormatter.ts](../atls-studio/src/services/batch/resultFormatter.ts) `tryBuildFileViewMergedPointer` | 3-slice read collapses from ~10k → ~250 tokens; FileView block becomes the single source of truth |
| TR6 | **Skip-archive predicate (Rule C)** — `isContentArchiveWorthy(content, toolName)` returns `false` for batch tool_results whose every line is a status line, dedupe tail, merge pointer, footer, or volatile nudge | [historyCompressor.ts](../atls-studio/src/services/historyCompressor.ts) | Both chunk-creation paths skip `addChunk`; no archived `batch` shell in the manifest for no-signal results |
| TR7 | **Repeated-misuse telemetry (Rule D)** — `recordBatchFailure` accumulates `(op, message)` buckets; BB `telemetry:failed-ops:session` written at `count >= 3`; surfaced in `session.debug` as `[REPEATED xN]` | [freshnessTelemetry.ts](../atls-studio/src/services/freshnessTelemetry.ts) | Preserves the learning signal that archived shells never carried; durable across compaction |
| TR8 | **Rolling-window eviction** — rounds beyond `ROLLING_WINDOW_ROUNDS` (20) spliced from history; tool-pairing invariant preserved via synthetic `tool_result` placeholders. No distillation — durable state already lives in BB / hash manifest / FileViews / `ru` rules | [historyCompressor.ts](../atls-studio/src/services/historyCompressor.ts) | Bounds verbatim history; zero synthesis overhead |
| TR9 | **`countSubstantiveRounds`** excludes synthetic auto-continue messages from round counting | [historyCompressor.ts](../atls-studio/src/services/historyCompressor.ts) | Rolling window ages out real rounds, not artifacts |
| TR10 | **Emergency compression path** in `compressToolLoopHistory` (skips protected rounds) | [historyCompressor.ts](../atls-studio/src/services/historyCompressor.ts) | Aggressive deflation when under pressure |
| TR11 | **Context hygiene middleware** at `COMPACT_HISTORY_TOKEN_THRESHOLD` (18k) / `COMPACT_HISTORY_TURN_THRESHOLD` (20 rounds) | [services/chatMiddleware.ts](../atls-studio/src/services/chatMiddleware.ts), [promptMemory.ts](../atls-studio/src/services/promptMemory.ts) | Safety net for extremely long sessions |
| TR12 | **`formatResult` size caps** (`FORMAT_RESULT_MAX_DEFAULT=80000`, `FORMAT_RESULT_MAX_SEARCH=120000`, `FORMAT_RESULT_MAX_GIT=100000`) | [utils/toon.ts](../atls-studio/src/utils/toon.ts) | Bounds handler output size entering the transcript |
...
- A 10-round session with deflation + rolling-window eviction holds verbatim history **bounded** at ~24k tokens regardless of how much was read.
