...
Beyond hash deflation, the compressor maintains a **verbatim window** of the most recent tool-loop rounds in history. The constant lives in [`promptMemory.ts`](../atls-studio/src/services/promptMemory.ts): `ROLLING_WINDOW_ROUNDS` (20).

When the number of **rounds** in history exceeds the window, the **oldest** round is spliced out — and nothing else happens. There is no distilled `[Rolling Summary]` message: durable cross-round state is already authoritatively carried by:

- **Blackboard** (keyed findings, survives compaction)
- **Hash manifest** (artifact index)
- **FileViews** (auto-healing edits)
- **`ru` rules** (policy)

Re-constructing a weak string-scraped distillation alongside those primitives was redundant at best and cache-poisoning at worst, so it was removed. The window still bounds total history size; eviction preserves the tool-pairing invariant (if an assistant `tool_use` loses its paired `tool_result`, a synthetic placeholder is re-inserted).

**Substantive round counting.** `countSubstantiveRounds` in [`historyCompressor.ts`](../atls-studio/src/services/historyCompressor.ts) excludes rounds triggered by synthetic auto-continue prompts (e.g. "Your response was truncated", "Continue working.") from the window count, so system-injected continuations don't push real work out of the verbatim transcript.

```mermaid
flowchart LR
  verbatim[Verbatim_history_window]
  evict[splice oldest round]
  bbFileViewsRules[BB + manifest + FileViews + rules]
  apiHist[Provider_message_list]
  verbatim -->|rounds_age_out| evict
  verbatim --> apiHist
  bbFileViewsRules -.->|authoritative cross-round state| apiHist
```

Snapshots persisted before v8 carry a `rollingSummary` payload; v8+ loads discard it (see [`session-persistence.md`](session-persistence.md)).
...
**Source**: [`historyCompressor.ts`](../atls-studio/src/services/historyCompressor.ts), [`chatMiddleware.ts`](../atls-studio/src/services/chatMiddleware.ts), [`contextHash.ts`](../atls-studio/src/utils/contextHash.ts) (`formatChunkRef`), [`promptMemory.ts`](../atls-studio/src/services/promptMemory.ts) (thresholds)
