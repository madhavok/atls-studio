...
| **annotate** | `annotate.note` (accepts `note` and/or `fields` — one verb for both free-form notes and metadata edits), `annotate.link`, `annotate.retype`, `annotate.split`, `annotate.merge`, `annotate.design` | Annotate, connect, and restructure engrams |
