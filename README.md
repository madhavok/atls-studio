...
- **Pinned-WM hygiene (ASSESS + retention-op compaction)** — Two complementary layers close the silent-accumulator gap. **ASSESS** is an ephemeral steering block that surfaces the oldest / largest pinned targets with a per-row `release | compact | hold` decision when CTX climbs or a pin survives edit-forwards untouched; single-fire dedupe keyed on candidates + CTX bucket. **Retention-op compaction** rewrites finalized `session.pin/unpin/drop/unload/compact/bb.delete` calls in history — tool_use args become `{n:3}`, OK per-step lines collapse to `ok`, FAIL lines stay verbatim for diagnostic signal. *Why:* pins outliving their usefulness are the dominant long-session bloat pattern, and the specific hashes in a prior round's retention tool_use are ghost refs the model re-emits by template. Measured 20.7% input-token savings over 20 sub-threshold retention rounds with byte-stable BP3 prefix.
